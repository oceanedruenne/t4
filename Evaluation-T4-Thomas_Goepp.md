...
- Plusieurs difficulté, qui dans une version plus facile nous permetterait d'avoir les informations sur les différentes barres d'informations qui seront modifiés.

- D'autres scénarios, des réponses différentes pour les même question, pour avoir un certain hasard mais toujours dans un but logique.
...
Justification: Thomas a été impliqué, bien qu'un peu tardivement, dans l'évaluation du projet, et a pu nous faire des propositions intéressantes
...
- Il n'y a pas de règles de nommages spécial, mais nous avons quand même des informations claires et précises pour la plupart des commits.

- Peut être un certain manque de commit, il y en a que 67, donc on peut se dire que certaines modifications ont été mise dans des commits pas destiné ou pas renseigné à cela.

- Le README n'est pas complet, il manque des captures d'écran, pour l'installation rien n'est expliqué mais étant donné que le projet est sur navigateur, il n'est pas nécéssaire d'avoir des explications.

- Le CDC sert beaucoup plus que le README, ils ne sont pas forcément liés mais on peut dire que le CDC nous donne des informations pour le README.
...
- Pour l'installation et l'execution tout est ok car c'est juste un lien et ça nous redirige directement vert le jeu sur le navigateur, rien a dire.

- Pour les instructions, un bouton "règles du jeu" est présent mais n'est pas fonctionnel.

- On comprend très bien le concept-cible au joueur avec les différentes barres d'informations.
...
- Pour la description de l'application c'est très intéressant d'avoir fait une simulation d'une partie, ça donne une idée de comment ce déroule la partie et arrivent les premiers événements.

- Le diagramme de classe me semble léger avec un manque d'informations.
...
- Il n'y a pas de descriptions des fonctionnalités car ce n'est pas nécessaire dans le concept actuel, tout est intuitif.(j'ai mis au en réponse moyenne car je sais pas si je dois mettre tout les points ou pas tout les points même si ce n'est pas nécéssaire)

- Pour la priorisation avec les différentes barres d'informations et la pensées du patients, nous pouvons comprendre et analysé les réponses qu'on doit dire au partient.

- Il n'y a pas non plus d'annotation car tout est simple et limpide (j'ai mis au en réponse moyenne car je sais pas si je dois mettre tout les points ou pas tout les points même si ce n'est pas nécéssaire)
...
- Le jeu ne propose pas de scénarios différents car il y a toujours les même questions dans le même ordre

- Mais même si les questions sont les même, en fonction de ce qu'on répond le patient réagis différement et nous devrons répondre d'une autre façon.
